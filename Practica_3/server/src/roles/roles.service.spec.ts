...
describe('RolesService', () => {
  ...
  describe('create', () => {
    it('should create a role when provided valid data', async () => {
      ...
      const mockCreatedRole = new Roles(); // Create a Role instance

      jest.spyOn(model.prototype, 'save').mockResolvedValueOnce(mockCreatedRole);
      jest.spyOn(model, 'findOne').mockResolvedValueOnce(null);
    ...
    it('should throw a ConflictException when a role with the same name already exists', async () => {
      ...
      jest.spyOn(model, 'findOne').mockResolvedValueOnce(new Roles()); // Simulate an existing role
  ...
  // Add other test cases for findAll, update, and remove here...
